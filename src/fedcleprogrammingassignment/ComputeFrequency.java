...
//   }
   ...
   Map<String,Integer> wordCount = new HashMap<>();
    ...
    //String[] wordArray = inputString.replace("[^a-zA-Z'\\s]"," ").split(" ");
    String word;
//    while((line = reader.readLine()) != null) {
//        //wordArray = line.split("\\s");
//        wordArray = line.split("\\b");
        ...
        //String countedWords = wordArray.toString();
          ...
          for (int i = 0; i < len; i++) {
                word = wordArray[i].toLowerCase();//.replaceAll(((?=.*\w)^(\w|')+$),"");
               // word = word.replaceAll(".*(?=\\b\\w|\\W)", "");//this was working
                 word = word.replaceAll("[^\\w']", "");//not usre  but I think this worked better
                    //word = word.replaceAll("\\b\\w*[-']\\w*\\b","");
            count = wordCount.get(word);
            if(count != null){
               count++; }else{
               count=1;}
